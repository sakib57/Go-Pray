...
export class Tab1Page {
  ...
  ionViewWillEnter(){
    ...
    this.storage.get('user_id').then((userID)=>{
      if(userID){
        this.newsService.getAllNews(0).subscribe(res=>{
          this.news = res.news
          this.noData = ''
  ...
  getUpcomingWakt(){
      ...
      //console.log('rrrrrrr',c_hr);
      //console.log('rrrrrrr',c_mn);
      if(this.Fajr){
  ...
  countDown(hr,mn){
    ...
    //console.log('Final',final_hr+':'+final_mn+':'+final_sc)
  ...
  loadData(event) {
    setTimeout(() => {
      ...
      if(this.length < this.news.length){
        if(this.search_term != ''){
          this.newsService.searchNews(this.search_term,this.i).subscribe(res=>{
            console.log('adding searched news...')
            this.news = this.news.concat(res.news)
            this.length += res.news.length
            console.log(this.length)
            console.log(this.news.length)

          })
        }else{
          this.newsService.getAllNews(this.i).subscribe(res=>{
            console.log('addinf all news....')
            this.news = this.news.concat(res.news)
            this.length += res.news.length
          })
        }
  ...
  search(v){
    ...
    this.length = 8
    this.search_term = v
    this.newsService.searchNews(v,this.i).subscribe(res=>{
      console.log(res)
      if(res.status == 200){
        this.news = res.news
        this.dataAvailable = true
